...
try: 
    ...
    replace = False # whether to replace the sampled events or not
print(f"{area}, Scoring number of heatwave days = {nb_heatw_day}, Maximum temperature = {temp_max}, Maximum temperature with anomaly = {temp_max_anom}, replace = {replace}")
...
boosted_data_around_peak=boosted_data_around_peak.stack(lead_ID=("case","lead_time"))
#keep all values
boosted_data = xr.open_mfdataset(files,concat_dim="case", combine="nested",parallel=True)
boosted_data["case"] = cases
boosted_data = boosted_data.stack(lead_ID=("case","lead_time"))
